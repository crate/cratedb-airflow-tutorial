...
def get_sp500_ticker_symbols():
    ...
    # the first section (index=0) in the components table contains the headers (which are unimportant in this context)
    # therefore, only data from index=1 on is taken.
    # moreover, each row in the table is stored in a <tr> division, so we find all of those. The data is stored in an array, where
    # each element contains information about a S&P 500 company.
    data_rows = components_table[0].find("tbody").find_all("tr")[1:]

    tickers = []
    
    # now the tickers are extracted from the data
